...
public static function creaCarpetaConFecha($directory){
	$carpeta = Acl::nowInEastern();
	$root = $_SERVER["DOCUMENT_ROOT"];
	$dir = $directory.$carpeta;
	if( !file_exists($dir) ) {
			mkdir($dir, 0755, true);
	}
...
public static function creaCarpetaSinFecha($directory){
	$root = $_SERVER["DOCUMENT_ROOT"];
	$dir = $directory;
	if( !file_exists($dir) ) {
			mkdir($dir, 0755, true);
	...
	public static function creaCarpetaGuardaArchivo($uploadedFile, $directory,$tipoFile){
		//$uploadedFiles = $request->getUploadedFiles();
							$directory=Acl::creaCarpetaConFecha($directory);
		    			...
		    			Archivo::NuevoRegistro($data);
		    			$tmp = Archivo::ObtenerRegistroByArray($data);
		    			return $tmp;
	...
	public static function creaCarpetaGuardaArchivoObjeto($uploadedFile, $directory){
		//$uploadedFiles = $request->getUploadedFiles();
							...
							$docAdjuntoObjeto = new Docadjunto($docAdjunto);
							$docAdjuntoObjeto->save();

							return $docAdjuntoObjeto;
	}

	public static function creaCarpetaGuardaArchivoObjetoActualizar($uploadedFile, $directory, $id){
		//$uploadedFiles = $request->getUploadedFiles();
							//crear por fecha para ordenar las cargas de las fotos
							$carpeta = Acl::nowInEastern();
							$root = $_SERVER["DOCUMENT_ROOT"];
							...
							if ($uploadedFile->getError() === UPLOAD_ERR_OK) {
								...
								Docadjunto::where('IDDOCADJUNTO', $id)
								->update($docAdjunto);
							}
							return Docadjunto::getDocAdjuntoByID($id);
	}
